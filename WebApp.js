...
function claimDailyReward(dateKey) {
    if (window.Telegram && Telegram.WebApp) {
        ...
        // Показываем попап и закрываем WebApp
        Telegram.WebApp.showPopup({
            title: 'Готово!',
            message: 'Награда отправлена на обработку. Проверьте чат с ботом для подтверждения и обновлённого баланса.',
            buttons: [{ id: 'close', type: 'default', text: 'Закрыть' }]
        }, function(buttonId) {
            if (buttonId === 'close') {
                Telegram.WebApp.close();  // Закрываем WebApp, пользователь возвращается в чат
            }
        });
    } else {
        // Режим тестирования
